...
//
// Chrome is always torn down, even on Ctrl+C or a thrown error. A stray GPU
// process from an interrupted run can sit at 100%+ CPU for days.
...
function killChrome() {
  ...
  try { process.kill(-pid, 'SIGKILL'); } catch { /* not a process group leader */ }
  try { process.kill(pid, 'SIGKILL'); } catch { /* already dead */ }
  chrome = null;
}

for (const sig of ['SIGINT', 'SIGTERM', 'SIGHUP']) {
  process.on(sig, () => { killChrome(); process.exit(128); });
}
process.on('exit', killChrome);

try {
  chrome = spawn(resolve(BIN), [
  ...
  await new Promise((ok, fail) => {
    ...
    chrome.on('exit', (code) => {
      if (!wsUrl) { clearTimeout(t); fail(new Error(`chrome exited early (${code})`)); }
    });
  });

  browser = await puppeteer.connect({ browserWSEndpoint: wsUrl, defaultViewport: { width: 1280, height: 720 } });
